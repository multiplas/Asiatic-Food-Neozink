	...
	function pgafu_plugin_row_meta( $links, $file ) {
		...
		if ( $file == PGAFU_PLUGIN_BASENAME ) {
			...
			$row_meta = array(
				...
				'support' => '<a href="' . esc_url(PGAFU_SITE_LINK . '/pricing/?utm_source=post_grid_filter&utm_medium=plugin_list&utm_campaign=plugin_quick_link') . '" title="' . esc_attr( __( 'go-premium', 'post-grid-and-filter-ultimate' ) ) . '" target="_blank">' . __( 'Go Premium', 'post-grid-and-filter-ultimate' ) . '</a>',
			);
